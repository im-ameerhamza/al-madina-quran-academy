<?php
$pageTitle = "Popular Quran Courses | Al Madinah Quran Academy";
$pageDescription = "Explore our online Quran courses including Nazra, Tajweed, Hifz, Qaida and Islamic studies for children and adults.";
$pageKeywords = "Quran Courses, Nazra Quran, Tajweed Course, Hifz Quran";
?>

<!DOCTYPE html>
<html class="no-js" lang="en">
  <head>
   <?php require_once 'includes/head.php'; ?>
  </head>

  <body class="body-pattern">
    <!--==============================
    Sidemenu
============================== -->
<?php require_once 'includes/sidemenu.php'; ?>

<!--==============================
Header Area
==============================-->
    <!-- Navbar -->
<?php require_once 'includes/navbar.php'; ?>

    <!--==============================
    Breadcumb
============================== -->
    <div class="breadcumb-wrapper" data-bg-src="assets/img/bg/courses-bg.webp">
      <div class="container">
        <div class="breadcumb-content">
          <h1 class="breadcumb-title">Popular courses</h1>
          <ul class="breadcumb-menu">
            <li><a href="index.php">Home</a></li>
            <li>Popular courses</li>
          </ul>
        </div>
      </div>
    </div>
    <!--==============================
cousrse Area
==============================-->
    <section
      class="overflow-hidden space overflow-hidden"
      style="background-image: url('assets/img/bg/about-bg-1.webp')"
    >
      <div class="container">
        <div class="row gy-4">
          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/norani-qaida.webp"
                  alt="Norani Qaida"
                  style="width: 476px; height: 260px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>500 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>4 Months Duration
                </div>
              </div>
              <span class="tags">Noorani Qaida</span>
              <h3 class="box-title">
                <a href="course-details.php?id=1">Madani Noorani Qaida</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/hafiz-ameer-hamza.webp"
                      alt="Avater"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Hafiz Ameer Hamza</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>
              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=1"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/nazra-quran.webp"
                  alt="Quran Image"
                  style="width: 476px; height: 260px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>500 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>12 Months Duration
                </div>
              </div>
              <span class="tags">Nazra Quran</span>
              <h3 class="box-title">
                <a href="course-details.php?id=2">Nazra Quran With Tajweed</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/rizwana-nazir.webp"
                      alt="Avater"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Rizwana Nazir</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>
              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=2"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/hafiz-quran.webp"
                  alt="Hafiz Quran"
                  style="width: 476px; height: 260px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>100 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>01 to 03 years
                  Duration
                </div>
              </div>
              <span class="tags">Hifz-e-Quran</span>
              <h3 class="box-title">
                <a href="course-details.php?id=3">Hifz-e-Quran with Tajweed</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img src="assets/img/team/ahmad-raza.webp" alt="Avater" />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Ahmad Raza</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>
              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=3"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/tarjuma-quran.webp"
                  alt="Quran Picture"
                  style="width: 476px; height: 260px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>150 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>12 Months Duration
                </div>
              </div>
              <span class="tags">Tarjmat ul Quran</span>
              <h3 class="box-title">
                <a href="course-details.php?id=4">Tarjmat ul Quran</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/abdul-quddoos.webp"
                      alt="Avater"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Abdul Quddoos Madni</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=4"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/tajweed-quran.webp"
                  alt="A person reading Quran"
                  style="width: 476px; height: 260px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>200 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>12 Months Duration
                </div>
              </div>
              <span class="tags">Tajweed</span>
              <h3 class="box-title">
                <a href="course-details.php?id=5">Tajweed Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/amina-shehbaz.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Amina Shehbaz</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=5"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/farz-uloom.webp"
                  alt="Quran Books"
                  style="width: 476px; height: 260px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>250 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>06 Months Duration
                </div>
              </div>
              <span class="tags">Farz-E-Uloom</span>
              <h3 class="box-title">
                <a href="course-details.php?id=6">Farz-E-Uloom Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/maryam-zahid.Webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Maryam Zahid</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=6"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/hadith-course1.webp"
                  alt="Quran Books"
                  style="width: 476px; height: 271px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>150 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>Depends of Students
                  (Duration)
                </div>
              </div>
              <span class="tags">The Hadith Course</span>
              <h3 class="box-title">
                <a href="course-details.php?id=7">The Hadith Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/hafiz-ameer-hamza.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Hafiz Ameer Hamza</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=7"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/darse-nizami.webp"
                  alt="A group people reading quran"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>100 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>04 Years Duration
                </div>
              </div>
              <span class="tags">Darse Nizami</span>
              <h3 class="box-title">
                <a href="course-details.php?id=8">Darse Nizami Short Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/hafiz-ameer-hamza.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Hafiz Ameer Hamza</h3>
                    <span class="box-desig"
                      >Islamic Scholar & Dars-e-Nizami</span
                    >
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=8"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/surah-noor-nisa-yusuf.webp"
                  alt="Surah Noor, Nisa, Yusuf"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>200 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>03 Months Duration
                </div>
              </div>
              <span class="tags">Tafseer Surah</span>
              <h3 class="box-title">
                <a href="course-details.php?id=9"
                  >Tafseer Surah Noor, Nisa, Yusuf</a
                >
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/amina-shehbaz.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Amina Shehbaz</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=9"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/tafseer-ul-jinan.webp"
                  alt="Surah Noor, Nisa, Yusuf"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>200 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>8 Week Duration
                </div>
              </div>
              <span class="tags">Tafseer Sirat ul Jinan</span>
              <h3 class="box-title">
                <a href="course-details.php?id=10"
                  >Tafseer Sirat ul Jinan Course</a
                >
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/sana-manzoor.Webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Sana Manzoor</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=10"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/aqaid-fiqh.webp"
                  alt="Surah Noor, Nisa, Yusuf"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>150 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>02 Years Duration
                </div>
              </div>
              <span class="tags">Aqaid o Fiqh</span>
              <h3 class="box-title">
                <a href="course-details.php?id=11">Aqaid-o-Fiqh Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/abdul-quddoos.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Abdul Quddoos</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=11"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/namaz.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>500 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>01 Month Duration
                </div>
              </div>
              <span class="tags">Namaz</span>
              <h3 class="box-title">
                <a href="course-details.php?id=12">Namaz Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/kabir-rafiq.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Kabir Rafiq</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=12"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/Sunnat-Nikah.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>500 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>01 Month Duration
                </div>
              </div>
              <span class="tags">Sunnat e Nikah</span>
              <h3 class="box-title">
                <a href="course-details.php?id=13">Sunnat e Nikah Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/rizwana-nazir.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Rizwana Nazir</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=13"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/gallery/Hajj.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>100 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>02 Months Duration
                </div>
              </div>
              <span class="tags">Hajj</span>
              <h3 class="box-title">
                <a href="course-details.php?id=14"
                  >Hajj Short & Complete Course</a
                >
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/abdul-quddoos.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Abdul Quddoos</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=14"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/umrah.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>200 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>02 Months Duration
                </div>
              </div>
              <span class="tags">Umrah</span>
              <h3 class="box-title">
                <a href="course-details.php?id=15"
                  >Umrah Short & Complete Course</a
                >
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/abdul-quddoos.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Abdul Quddoos</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=15"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/gallery/Ramadan.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>300 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>01 Month Duration
                </div>
              </div>
              <span class="tags">Ahkam e Roza</span>
              <h3 class="box-title">
                <a href="course-details.php?id=16">Ahkam e Roza Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/sana-manzoor.Webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Sana Manzoor</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=16"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/islamic-life.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>500 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>01 Month Duration
                </div>
              </div>
              <span class="tags">Islamic Life</span>
              <h3 class="box-title">
                <a href="course-details.php?id=17">Islamic Life Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/kabir-rafiq.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Kabir Rafiq</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=17"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/seerat-e-mustafa.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>300 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>06 Months Duration
                </div>
              </div>
              <span class="tags">Seerat e Mustafa</span>
              <h3 class="box-title">
                <a href="course-details.php?id=18">Seerat e Mustafa Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/hafiz-ameer-hamza.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Hafiz Ameer Hamza</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>
              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=18"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/sunat-course.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>250 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>01 Month Duration
                </div>
              </div>
              <span class="tags">Sunnatain & Adab</span>
              <h3 class="box-title">
                <a href="course-details.php?id=19">Sunnatain & Adab Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/ahmad-raza.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Ahmad Raza</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=19"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/taharat.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>250 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>01 Month Duration
                </div>
              </div>
              <span class="tags">Taharat</span>
              <h3 class="box-title">
                <a href="course-details.php?id=20">Taharat Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/abdul-quddoos.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Abdul Quddoos</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=20"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>

          <div class="col-lg-6 col-xxl-4">
            <div class="cousrse-card style2">
              <div class="box-img global-img">
                <img
                  src="assets/img/cousrse/arabic-grammar.webp"
                  alt="A person is offering namaz"
                  style="width: 476px; height: 270px"
                />
              </div>
              <div class="meta">
                <span class="icon"
                  ><i class="fa-solid fa-user-graduate"></i>250 Enrolled</span
                >
                <div class="icon">
                  <i class="fa-sharp fa-light fa-clock"></i>06 Months Duration
                </div>
              </div>
              <span class="tags">Arabic Grammar</span>
              <h3 class="box-title">
                <a href="course-details.php?id=21">Arabic Grammar Course</a>
              </h3>
              <div class="box-wrapp">
                <div class="box-profile">
                  <div class="box-author">
                    <img
                      src="assets/img/team/amina-shehbaz.webp"
                      alt="Islamic Scholar"
                    />
                  </div>
                  <div class="box-info">
                    <h3 class="box-title">Amina Shehbaz</h3>
                    <span class="box-desig">Islamic Scholar</span>
                  </div>
                </div>
                <span class="rating"><i class="fa-solid fa-star"></i>4.8</span>
              </div>

              <div class="btn-group justify-content-between">
                <a class="th-btn border-btn2" href="course-details.php?id=21"
                  >Learn More</a
                >
                <a href="contact.php" class="th-btn"
                  ><span
                    class="btn-text"
                    data-back="Enroll Now"
                    data-front="Enroll Now"
                  ></span>
                </a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!--==============================
	Footer Area
==============================-->
    <?php require_once 'includes/footer.php'; ?>


    <!--==============================
    All Js File
============================== -->
  <?php require_once 'includes/script.php'; ?>
  </body>
</html>
